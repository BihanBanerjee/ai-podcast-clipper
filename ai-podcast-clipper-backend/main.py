...
class ProcessVideoRequest(BaseModel):
    s3_key: str
    mode: str = "question"  # Default to question mode
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    print(f"Output S3 key: {output_s3_key}") # Output S3 key: test1/clip_0.mp4
    ...
    s3_client.upload_file(
        subtitle_output_path, "ai-podcast-clipping", output_s3_key)
...
class AiPodcastClipper:
    ...
    def identify_moments(self, transcript: dict, mode: str = "question"):
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        mode = request.mode  # Get the mode from request
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipping", s3_key, str(video_path))
        ...
        identified_moments_raw = self.identify_moments(transcript_segments, mode)
